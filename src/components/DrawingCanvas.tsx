...
export default function DrawingCanvas() {
  ...
  const handlePanelClick = (e: React.MouseEvent, panelId: string) => {
    ...
    if (isShiftPressed) {
      ...
      setOriginalZIndex(prev => ({
        ...prev,
        [panelId]: prev[panelId] ?? panel.zIndex,
      }));
  ...
  const handleCanvasClick = () => {
    ...
    if (singleSelectedPanel) {
      ...
      if (original !== undefined) {
        updatePanel(singleSelectedPanel, { zIndex: original });
      }
    }
    // setSingleSelectedPanel([]);
    setSelectedPanels([]); // optional for multi-select
    setOriginalZIndex({});
  ...
  function getShapeProps(panel: Panel) {
    return {
      ...
      className: `transition-transform duration-300 rotate(${panel.rotation || 0}deg)`
    };
  }

  const getPanelBorderStyle = (panelId: string) => {
    if (singleSelectedPanel === panelId) {
      return {
            ...
            >
              {panels.map(panel => (
                <Rnd
                  ...
                  disableDragging={panel.isLocked}
                  style={{
