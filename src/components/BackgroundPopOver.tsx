...
const BackgroundPopover = ({onBgImageChange}: BackgroundPopoverProps) => {
...
const categories = ["energy","midnight","sunset","radiant","iridescent","spring"];
  ...
  return (
    <div className="relative inline-block">
      ...
      >
        {selectedBgLink ? (<img src={selectedBgLink} alt="Bg" className='w-4 h-4'/>): (<svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                {Array.from({length: 30},(_,id) => {
                  ...
                  <button
                    onClick={() => {
                        ...
                        onBgImageChange(`https://rkaznxeczerlxbno.public.blob.vercel-storage.com/backgrounds/${categories[categories.indexOf(currentCategory)]}/bpxl_wp00${categories.indexOf(currentCategory)+1}_${String(id+1).padStart(2, '0')}.webp`)
                        setSelectedBgLink(`https://rkaznxeczerlxbno.public.blob.vercel-storage.com/backgrounds/${categories[categories.indexOf(currentCategory)]}/bpxl_wp00${categories.indexOf(currentCategory)+1}_${String(id+1).padStart(2, '0')}.webp`)
                    }}
                    className={`w-full h-16 rounded-lg overflow-clip border-2 transition-all ${
                  ...
                  >
                    <img src={`https://rkaznxeczerlxbno.public.blob.vercel-storage.com/backgrounds/${categories[categories.indexOf(currentCategory)]}/bpxl_wp00${categories.indexOf(currentCategory)+1}_${String(id+1).padStart(2, '0')}.webp`} alt=""/>
                    {selectedBackground === String(currentCategory+id+1) && (
